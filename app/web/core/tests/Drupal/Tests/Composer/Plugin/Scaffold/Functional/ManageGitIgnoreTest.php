...
/**
 * Tests to see whether .gitkeep files are correctly managed.
 *
...
class ManageGitIgnoreTest extends TestCase {
  ...
  protected function createSutWithGit($fixture_name) {
    ...
    // .gitkeep files will not be managed unless there is a git repository.
  ...
  /**
   * Tests scaffold command correctly manages the .gitkeep file.
   */
  public function testManageGitIgnore() {
    // Note that the drupal-composer-drupal-project fixture does not
    // have any configuration settings related to .gitkeep management.
    ...
    $this->assertFileNotExists($sut . '/docroot/sites/.gitkeep');
...
/web.config
EOT;
    // At this point we should have a .gitkeep file, because although we did
    // not explicitly ask for .gitkeep tracking, the vendor directory is not
    // tracked, so the default in that instance is to manage .gitkeep files.
    $this->assertScaffoldedFile($sut . '/docroot/.gitkeep', FALSE, $expected);
    $this->assertScaffoldedFile($sut . '/docroot/sites/.gitkeep', FALSE, 'example.settings.local.php');
    $this->assertScaffoldedFile($sut . '/docroot/sites/default/.gitkeep', FALSE, 'default.services.yml');
    $expected = <<<EOT
M docroot/.gitkeep
?? docroot/sites/.gitkeep
?? docroot/sites/default/.gitkeep
EOT;
    // Check to see whether there are any untracked files. We expect that
    // only the .gitkeep files themselves should be untracked.
    $stdout = $this->mustExec('git status --porcelain', $sut);
    $this->assertEquals(trim($expected), trim($stdout));
  }

  /**
   * Tests scaffold command does not manage the .gitkeep file when disabled.
   */
  public function testUnmanagedGitIgnoreWhenDisabled() {
    // Note that the drupal-drupal fixture has a configuration setting
    // `"gitignore": false,` which disables .gitkeep file handling.
    ...
    $this->assertFileNotExists($sut . '/.gitkeep');
    $this->assertFileNotExists($sut . '/docroot/sites/default/.gitkeep');
  }
   ...
   * .gitkeep files, then we expect that the unmanaged file should not be
   * added to the .gitkeep file, because unmanaged files should be committed.
   */
  public function testAppendToEmptySettingsIsUnmanaged() {
    ...
    $this->assertFileNotExists($sut . '/sites/.gitkeep');
    // Run the scaffold command.
    $this->fixtures->runScaffold($sut);
    $this->assertFileExists($sut . '/autoload.php');
    $this->assertFileExists($sut . '/index.php');

    $this->assertScaffoldedFile($sut . '/sites/.gitkeep', FALSE, 'example.sites.php');
    $this->assertScaffoldedFileDoesNotContain($sut . '/sites/.gitkeep', 'settings.php');
  }

  /**
   * Tests scaffold command disables .gitkeep management when git not present.
   *
  ...
  public function testUnmanagedGitIgnoreWhenGitNotAvailable() {
    ...
    // configuration settings related to .gitkeep management.
    $sut = $this->createSutWithGit('drupal-composer-drupal-project');
    $this->assertFileNotExists($sut . '/docroot/sites/default/.gitkeep');
    $this->assertFileNotExists($sut . '/docroot/index.php');
    $this->assertFileNotExists($sut . '/docroot/sites/.gitkeep');
